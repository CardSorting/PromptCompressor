...
export class BroccoliCodeAstSymbolicDedupBuffer {
    static instance;
    codeAuditTable;
    static LICENSE_HEADER_REGEX = /\/\*[\s\S]*?(?:Copyright|License|All\s+Rights\s+Reserved|Apache\s+License|MIT\s+License)[\s\S]*?\*\//gi;
    static IMPORT_STATEMENT_REGEX = /^(?:import\s+(?:[\w*\s{},$]+)\s+from\s+['"][^'"]+['"]|const\s+[\w\s{},$]+\s*=\s*require\([^)]+\));?/gm;
    constructor() {
     ...
     */
    static deduplicateCode(codeText) {
...
//# sourceMappingURL=BroccoliCodeAstSymbolicDedupBuffer.js.map