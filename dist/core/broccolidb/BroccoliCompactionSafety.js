...
export class BroccoliCompactionSafety {
    ...
    static verify(source, compacted) {
        if (source === compacted) {
        ...
        const unsupportedFacts = evidence
            .filter(item => !item.candidates.some(candidate => normalizedSource.includes(candidate.toLowerCase()) || sourceNumbers.has(candidate)))
    ...
    static extractHighRiskEvidence(value) {
        const evidence = [];
        this.collect(value, /\b[0-9]+(?:\.[0-9]+)?\s*%/g, evidence, match => this.numericEvidence(match[0], match[0]));
        this.collect(value, /(?:\$|\b(?:USD|EUR|GBP|CAD|AUD|JPY|CHF)\s*)[0-9][0-9,.]*/gi, evidence, match => this.numericEvidence(match[0], match[0]));
        this.collect(value, /\b[0-9]+(?:\.[0-9]+)?\s*(?:ms|µs|ns|gb|mb|kb|gbps|mbps|req\/s|rps|ops\/s|mg\/dl|mmhg|ml\/min|kg|kw|mw|mwh|gwh|psi|bar|sq\s*ft|shares|units)\b/gi, evidence, match => this.numericEvidence(match[0], match[0]));
        ...
        this.collect(value, /\b(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:t(?:ember)?)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\s+(\d{1,2}),\s+(\d{4})\b/gi, evidence, match => {
    ...
    static collect(value, regex, target, mapper) {
    ...
    static numericEvidence(display, rawValue) {
    ...
    static extractSourceNumbers(value) {
...
//# sourceMappingURL=BroccoliCompactionSafety.js.map