...
export class BroccoliEarningsCallCompactor {
     ...
     */
    static compactEarningsCall(rawTranscriptText) {
        ...
        const resParts = [];
        if (revMatch)
            resParts.push(`Revenue: ${revMatch[1].trim()}`);
        if (epsMatch)
            resParts.push(`EPS: ${epsMatch[1].trim()}`);
        if (marginMatch)
            resParts.push(`Margin: ${marginMatch[1].trim()}`);
        ...
        if (rawQna) {
            ...
            for (const turn of turns) {
                ...
                if (/^(?:Good morning|Thanks for taking|Congrats|Thank you)/i.test(trimmed))
                    continue;
...
//# sourceMappingURL=BroccoliEarningsCallCompactor.js.map