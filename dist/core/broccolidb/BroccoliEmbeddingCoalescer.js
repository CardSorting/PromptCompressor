...
export class BroccoliEmbeddingCoalescer {
     ...
     */
    static coalesceChunkBatch(chunks) {
        const coalescer = this.getInstance();
        const totalInputChunks = chunks.length;
        let originalTokens = 0;
        let uniqueTokens = 0;
        const uniqueChunksMap = new Map(); // hash -> chunkText
        const uncachedChunksToEmbed = [];
     ...
     */
    static storeEmbeddings(chunks, embeddings) {
        const coalescer = this.getInstance();
...
//# sourceMappingURL=BroccoliEmbeddingCoalescer.js.map