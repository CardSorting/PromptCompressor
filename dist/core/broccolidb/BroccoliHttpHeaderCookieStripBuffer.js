...
export class BroccoliHttpHeaderCookieStripBuffer {
    static instance;
    tokenCasStore = new Map();
    httpAuditTable;
    static NOISY_HEADER_REGEX = /^(?:User-Agent|Sec-Ch-Ua[\w-]*|Accept(?:-Language|-Encoding|-Charset)?|Connection|Keep-Alive|Upgrade-Insecure-Requests|X-Forwarded-For|X-Amzn-Trace-Id|X-Request-Id|X-Correlation-Id):\s*.+$/gim;
    static BEARER_JWT_REGEX = /Bearer\s+(eyJ[a-zA-Z0-9_-]+\.eyJ[a-zA-Z0-9_-]+\.[a-zA-Z0-9_-]+)/g;
    static COOKIE_HEADER_REGEX = /^Cookie:\s*(.+)$/gim;
    constructor() {
     ...
     */
    static deduplicateHttpHeaders(rawHttpLog) {
        ...
        cleaned = cleaned.replace(this.COOKIE_HEADER_REGEX, (_, cookieVal) => {
            strippedCount++;
            const cookieKeys = cookieVal.split(';').map((c) => c.trim().split('=')[0]);
...
//# sourceMappingURL=BroccoliHttpHeaderCookieStripBuffer.js.map