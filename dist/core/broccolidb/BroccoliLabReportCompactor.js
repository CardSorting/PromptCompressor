...
export class BroccoliLabReportCompactor {
     ...
     */
    static compactLabReport(tests) {
        const compactor = this.getInstance();
        // Estimate raw un-compacted verbose table tokens
        const rawVerboseRows = tests.map((t) => `Test: ${t.testName} | Result: ${t.value} ${t.unit} | Reference Range: ${t.referenceRange} | Status: ${t.flag || 'NORMAL'}`);
        const rawReport = rawVerboseRows.join('\n');
        const originalTokens = Math.ceil(rawReport.length / 4);
        const abnormalTests = [];
        const normalSummaries = [];
        for (const t of tests) {
            const isAbnormal = t.flag === 'HIGH' ||
                t.flag === 'LOW' ||
        ...
        if (abnormalTests.length > 0) {
            ...
            for (const ab of abnormalTests) {
                outputLines.push(`- [${ab.flag}] ${ab.testName}: ${ab.value} ${ab.unit} (Ref: ${ab.referenceRange})`);
            }
        }
        if (normalSummaries.length > 0) {
            outputLines.push(`[WITHIN NORMAL LIMITS (${normalSummaries.length} tests): ${normalSummaries.join(', ')}]`);
        }
...
//# sourceMappingURL=BroccoliLabReportCompactor.js.map