...
export class BroccoliLegalCitationCompactor {
     ...
     */
    static compactCitations(briefText) {
        ...
        text = text.replace(/Title\s+(\d+)\s+(?:of the )?United States Code(?:,\s*Section|\s*§)\s+([0-9a-zA-Z\(\)]+)/gi, '[STATUTE: $1 U.S.C. § $2]');
        // 2. Delaware General Corporation Law
        // e.g. "Delaware General Corporation Law, Section 102(b)(7)" -> "[STATUTE: DGCL § 102(b)(7)]"
        text = text.replace(/Delaware General Corporation Law(?:,\s*Section|\s*§)\s+([0-9a-zA-Z\(\)]+)/gi, '[STATUTE: DGCL § $1]');
        ...
        text = text.replace(/([A-Z][a-zA-Z\s\.,]+)\s+v\.\s+([A-Z][a-zA-Z\s\.,]+),\s*(\d+\s+U\.S\.\s+\d+)(?:,\s*\d+\s+S\.\s*Ct\.\s*\d+)?(?:,\s*\d+\s+L\.\s*Ed\.\s*2d\s*\d+)?\s*\(([0-9]{4})\)/g, '[CASE: $1 v. $2, $3 ($4)]');
...
//# sourceMappingURL=BroccoliLegalCitationCompactor.js.map