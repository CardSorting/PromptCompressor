...
export class BroccoliRealEstateCompactor {
     ...
     */
    static compactProperty(rawPropertyText) {
        ...
        const specsParts = [];
        if (bedsBathsMatch)
            specsParts.push(bedsBathsMatch[1].trim());
        if (sqftMatch)
            specsParts.push(sqftMatch[1].trim());
        if (yearMatch)
            specsParts.push(`Built ${yearMatch[1].trim()}`);
        ...
        const finParts = [];
        if (capMatch)
            finParts.push(`Cap Rate: ${capMatch[1].trim()}`);
        if (noiMatch)
            finParts.push(`NOI: ${noiMatch[1].trim()}`);
        if (hoaMatch)
            finParts.push(`HOA: ${hoaMatch[1].trim()}`);
...
//# sourceMappingURL=BroccoliRealEstateCompactor.js.map