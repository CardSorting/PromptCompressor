...
export class BroccoliSQLPlanCache {
    ...
    constructor() {
        ...
        this.registerPlan('TOP_CUSTOMERS_BY_COUNTRY', 'SELECT customer_id, name, SUM(amount) AS total_spend FROM orders WHERE country = $country AND year = $year GROUP BY customer_id, name ORDER BY total_spend DESC LIMIT $limit;', ['country', 'year', 'limit']);
        this.registerPlan('ACTIVE_SEATS_BY_ORG', 'SELECT org_id, tier, active_seats, billing_status FROM organizations WHERE org_id = $org_id;', ['org_id']);
    }
    static getInstance() {
    ...
    registerPlan(intentKey, templateSql, parameterNames) {
     ...
     */
    static matchAndExecute(naturalQuery, estimatedSchemaTokens = 1200, estimatedSqlOutputTokens = 80) {
        ...
        if (topCustMatch) {
            ...
            if (plan) {
                ...
                const dollarsSavedUsd = (estimatedSchemaTokens / 1_000_000) * 2.50 +
...
//# sourceMappingURL=BroccoliSQLPlanCache.js.map