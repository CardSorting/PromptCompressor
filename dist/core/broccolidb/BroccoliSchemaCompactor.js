...
export class BroccoliSchemaCompactor {
     ...
     */
    static compactSchema(schemaDef, options = {}) {
        ...
        const clonedSchema = JSON.parse(JSON.stringify(schemaDef));
        const pruneNode = (node, keyName) => {
            ...
            if (node.definitions) {
                for (const defVal of Object.values(node.definitions))
                    pruneNode(defVal);
            }
            if (node.$defs) {
                for (const defVal of Object.values(node.$defs))
                    pruneNode(defVal);
            }
        };
...
//# sourceMappingURL=BroccoliSchemaCompactor.js.map