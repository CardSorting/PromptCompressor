...
export class BroccoliSqlDdlSchemaHoistingBuffer {
    static instance;
    sqlAuditTable;
    static COMMON_AUDIT_COLS_REGEX = /,\s*(?:created_at|updated_at)\s+TIMESTAMP(?:TZ)?\s+(?:NOT\s+NULL\s+)?DEFAULT\s+(?:NOW\(\)|CURRENT_TIMESTAMP)/gi;
    static UUID_PK_REGEX = /id\s+UUID\s+PRIMARY\s+KEY\s+DEFAULT\s+(?:gen_random_uuid\(\)|uuid_generate_v4\(\)),?/gi;
    constructor() {
     ...
     */
    static deduplicateSqlDdl(sqlText) {
...
//# sourceMappingURL=BroccoliSqlDdlSchemaHoistingBuffer.js.map