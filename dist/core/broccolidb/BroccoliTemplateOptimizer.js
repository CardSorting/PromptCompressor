...
export class BroccoliTemplateOptimizer {
     ...
     */
    static renderTemplate(templateName, rawTemplate, variables) {
        const optimizer = this.getInstance();
        // 1. Evaluate Conditionals: {{#if var}}content{{else}}alternate{{/if}}
        let rendered = rawTemplate.replace(/\{\{#if\s+(\w+)\}\}([\s\S]*?)(?:\{\{else\}\}([\s\S]*?))?\{\{\/if\}\}/g, (_, varName, trueBranch, falseBranch = '') => {
...
//# sourceMappingURL=BroccoliTemplateOptimizer.js.map