 ...
 */
import { OpenAiRateCardRegistry, } from './ProviderRateCards.js';
import { OpenAiTaskSuite, } from './OpenAiTaskSuite.js';
...
export class BroccoliInferenceBoundaryBenchmarker {
    ...
    static async runSuite(options = {}) {
        ...
        const execMode = apiKey ? 'LIVE_OPENAI_API' : 'CALIBRATED_OFFLINE_SIMULATION';
        const provenance = {
     ...
     */
    static async evaluateTask(task, modelId, apiKey) {
        const execMode = apiKey ? 'LIVE_OPENAI_API' : 'CALIBRATED_OFFLINE_SIMULATION';
        // 1. Baseline Context Preparation
        const rawContextStr = Array.isArray(task.rawInputContext)
            ? task.rawInputContext.map((c) => typeof c === 'string' ? c : JSON.stringify(c)).join('\n')
        ...
        const resData = compactedResponse.result || compactedResponse.data;
        if (resData) {
     ...
     */
    static async callLiveOpenAi(apiKey, modelId, prompt) {
     ...
     */
    static simulateModelExecution(task, prompt, modelId, mode) {
     ...
     */
    static classifyEmpiricalRegime(savingsPercentage) {
        if (savingsPercentage >= 70)
            return 'High Redundancy Regime (>70% savings)';
        if (savingsPercentage >= 40)
            return 'Moderate Redundancy Regime (40-70% savings)';
        if (savingsPercentage >= 10)
            return 'Dense Information Regime (10-40% savings)';
        if (savingsPercentage >= 0)
            return 'Marginal Compression Regime (0-10% savings)';
     ...
     */
    static async evaluateBlindedWorkload(task, modelId = 'gpt-4o', apiKey) {
...
//# sourceMappingURL=BroccoliInferenceBoundaryBenchmarker.js.map