...
export type GalxRotationStrategy = 'adaptive_headroom' | 'smooth_weighted_round_robin' | 'least_utilized' | 'priority_failover';
...
export type PrivacyProfileType = 'standard' | 'hipaa' | 'hipaa_strict' | 'gdpr' | 'gdpr_eu' | 'lgpd_brazil' | 'pipeda_canada' | 'appi_japan' | 'australia_privacy' | 'swiss_fadp' | 'singapore_pdpa' | 'zdr' | 'confidential' | 'strict' | 'fintech_pci' | 'maximum_shield';
...
export interface DSARDataExport {
    ...
    readonly auditLogReferences: Array<{
        id: string;
        action: string;
        timestamp: string;
    }>;
...
export interface ShardAcquisitionResult {
    ...
    readonly status?: 'tenant_pool_quota_capped' | 'all_shards_cooldown' | 'no_healthy_shards' | 'success' | 'pool_exhausted' | 'privacy_policy_mismatch' | 'no_compliant_shards';
}
...
export interface HIPAAReidentificationRiskResult {
    ...
    readonly reidentificationRiskScore: number;
    readonly meetsExpertDeterminationThreshold: boolean;
    readonly quasiIdentifiersDetected: Array<{
        type: string;
        snippet: string;
        entropyScore: number;
    }>;
...
export interface SecurityProbeResult {
    ...
    readonly mitigationRatePercent: number;
...
//# sourceMappingURL=GalxContracts.d.ts.map