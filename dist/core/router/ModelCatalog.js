 ...
 */
export const EXCLUSIVE_MODEL_CATALOG = [
 ...
 */
export const FRONTEND_VISIBLE_MODELS = EXCLUSIVE_MODEL_CATALOG;
export class ModelCatalog {
    static catalog = new Map(EXCLUSIVE_MODEL_CATALOG.map(m => [m.id, m]));
    static fastPathCache = new Map();
    /**
    ...
    static computeResolution(requestedModel) {
        ...
        const upstreamMatch = EXCLUSIVE_MODEL_CATALOG.find(m => m.upstreamId.toLowerCase() === normalized);
        if (upstreamMatch)
    ...
    static listFrontendModels() {
        return FRONTEND_VISIBLE_MODELS;
    }
    static getSupportedModels() {
        return EXCLUSIVE_MODEL_CATALOG;
    }
    static listByProvider(provider) {
        return this.getTable().query({ where: { provider: { $eq: provider } } });
    }
    static listByCategory(category) {
        return this.getTable().query({ where: { category: { $eq: category } } });
    }
    static getSpec(id) {
        return this.resolveModel(id);
    }
    // --- BroccoliDB Zenith Integration ---
    static dbTableInstance = null;
    static getTable() {
        if (!this.dbTableInstance) {
            const table = new BroccoliDbTable('models');
    ...
    static queryNatural(naturalQuery) {
        ...
        return this.getTable().query(parsed.queryOptions);
    }
    /**
     * Evaluates rich operator filters ($gt, $between, $in, $regex) over the catalog.
     */
    static queryDSL(options) {
        return this.getTable().query(options);
    }
    /**
     ...
     */
    static calculateCost(modelId, usage) {
